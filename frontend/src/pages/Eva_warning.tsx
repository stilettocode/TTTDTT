...
import type { RoverData } from '../types';
...
// ══════════════════════════════════════════════════════════════════════
// ROVER (PR) TELEMETRY RANGES (from rover-telemetry-ranges.pdf)
// No rate limits for rover — threshold and nominal checks only
// ══════════════════════════════════════════════════════════════════════
const ROVER_RANGES: Record<string, { min?: number; max?: number; nominal?: number; unit?: string }> = {
  pitch:              { min: -50,   max: 50,    unit: 'deg' },
  roll:               { min: 0,     max: 50,    unit: 'deg' },
  speed:              { min: 0,     max: 18,    unit: 'm/s' },
  throttle:           { min: 0,     max: 100,   unit: '%' },
  steering:           { min: -1,    max: 1 },
  surface_incline:    { min: -50,   max: 50,    unit: 'deg' },
  distance_from_base: { min: 0,     max: 2500,  unit: 'm' },
  oxygen_tank:        { min: 25,    max: 100,   unit: '%' },
  oxygen_pressure:    { min: 2997,  max: 3000,  unit: 'psi' },
  fan_pri_rpm:        { min: 29999, max: 30005, unit: 'rpm' },
  fan_sec_rpm:        { min: 29999, max: 30005, unit: 'rpm' },
  cabin_pressure:     { min: 3.5,   max: 4.10,  nominal: 4.00, unit: 'psi' },
  cabin_temperature:  { min: 10,    max: 21,    unit: '°C' },
  coolant_pressure:   { min: 495,   max: 501,   nominal: 500,  unit: 'psi' },
  coolant_storage:    { min: 80,    max: 100,   nominal: 100,  unit: '%' },
  battery_level:      { min: 30,    max: 100,   unit: '%' },
}
...
function checkRanges(
  ...
  dtSec: number,
  ranges: Record<string, { min?: number; max?: number; nominal?: number; unit?: string }> = RANGES,
  rateLimits: Record<string, number> | null = RATE_LIMITS,
): AlertItem[] {
  ...
  for (const [key, range] of Object.entries(ranges)) {
    ...
    if (range.min !== undefined && val < range.min) {
      alerts.push({
        ...
        sub: `Value ${val.toFixed(2)}${unitStr} below min ${range.min}${unitStr}`,
        level: 'danger', ts: now,
      })
      ...
      alerts.push({
        ...
        sub: `Value ${val.toFixed(2)}${unitStr} exceeds max ${range.max}${unitStr}`,
        level: 'danger', ts: now,
      })
    } else if (range.nominal !== undefined && Math.abs(val - range.nominal) > 0.001) {
      // Caution: within bounds but off nominal
      alerts.push({
        id: `${label}-${key}-nominal`,
        title: `${label} ${key.replace(/_/g, ' ')} off nominal`,
        sub: `${val.toFixed(2)}${unitStr} (nominal: ${range.nominal}${unitStr})`,
        level: 'warn', ts: now,
      })
    }

    // Rate-of-change check — only when rateLimits provided and non-null
    if (rateLimits && prev && dtSec > 0) {
...
export default function Eva1Page() {
  const { evaData, roverData } = useSocket()
  ...
  useEffect(() => {
    ...
    // Rover data arrives as the pr_telemetry flat object
    const roverTelemetry: RoverData | null = roverData ?? null
    console.log("Rover telemetry:", roverTelemetry)
    console.log("ROVER_RANGES keys:", Object.keys(ROVER_RANGES))
    console.log("roverTelemetry keys:", roverTelemetry ? Object.keys(roverTelemetry) : 'null')

    const prAlerts = roverTelemetry?.pr_telemetry ? checkRanges('PR', roverTelemetry.pr_telemetry as any, prevRoverRef.current, dtSec, ROVER_RANGES, null) : []
    console.log("Rover alerts:", prAlerts)
    console.log("Data passed to checkRanges:", roverTelemetry?.pr_telemetry ? Object.entries(roverTelemetry.pr_telemetry).slice(0, 5) : 'null')
    const newAlerts: AlertItem[] = [
      // EVA checks — with rate limits
      ...checkRanges('EVA-1', eva1 as any, prevEva1Ref.current as any, dtSec, RANGES, RATE_LIMITS),
      ...checkRanges('EVA-2', eva2Normalized as any, prevEva2Ref.current as any, dtSec, RANGES, RATE_LIMITS),
      // Rover checks — no rate limits (pass null)
      ...(roverTelemetry?.pr_telemetry ? checkRanges('PR', roverTelemetry.pr_telemetry as any, prevRoverRef.current, dtSec, ROVER_RANGES, null) : []),
    ]
    ...
    prevRoverRef.current = roverTelemetry?.pr_telemetry ? { ...roverTelemetry.pr_telemetry } as unknown as Record<string, number> : null
  ...
  const backendConnected = Boolean(evaData) || Boolean(roverData)
