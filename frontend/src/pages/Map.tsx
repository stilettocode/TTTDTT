...
function MatrixOverlay({ matrix }: { matrix: MatrixUpdate | null }) {
  ...
  useEffect(() => {
    ...
    const rowHeight = IMAGE_HEIGHT / matrix.length
    matrix.forEach((row, rowIndex) => {
      if (!row.length) return

      const cellWidth = IMAGE_WIDTH / row.length
      ctx.font = `700 ${Math.max(1, Math.min(cellWidth, rowHeight) * 0.72)}px monospace`
      ...
      row.forEach((rawValue, colIndex) => {
        ...
        ctx.fillStyle = color
        ctx.fillRect(left, top, cellWidth, rowHeight)

        ctx.fillStyle = value === 3 || value === 5 ? '#111827' : '#f8fafc'
        ctx.fillText(String(value), left + cellWidth / 2, top + rowHeight / 2)
      })
    })
  }, [matrix])
          ...
          <TransformComponent wrapperStyle={{ width: '100%', height: '100%' }}>
            ...
            >
              ...
              >
                ...
                />
                <MatrixOverlay matrix={matrixUpdate} />
                <WaypointMarkers
                  activeWaypointId={activeWaypointId}
                  onSelect={setActiveWaypointId}
                  waypoints={waypoints}
